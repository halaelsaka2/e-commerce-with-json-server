...
let url = "/products";
const getAll = () => {
  return axiosInstance.get(`${url}?_page=${1}`);
};
...
const getProductsByfilters = async (category, color, priceRange, size, types, currentPage, limit, sort) => {
  ...
  if (color) Url = Url + `colors_like=${color}&`;

  if (limit) Url = Url + `_limit=${limit}&`;

  if (sort) Url = sort === "new" ? Url + `_sort=status&_order=asc&` : Url + `_sort=status&_order=desc&`;

  if (size) Url = Url + `sizes_like=${size}&`;
